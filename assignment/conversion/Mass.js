...
// 킬로그램
const gOrMl = (weight) => {
  const 그램 = `${(weight * 1000).toFixed(2)}g`;
  const 밀리그램 = `${(weight * 1e6).toFixed(2)}mg`; // 1e+6 : 1에서 오른쪽으로 여섯칸 이동 즉, 1000000
...
// 그램
const kgOrMl = (weight) => {
  const 킬로그램 = `${(weight / 1000).toFixed(2)}kg`;
  const 밀리그램 = `${(weight * 1000).toFixed(2)}mg`;
...
// 밀리그램
const kgOrg = (weight) => {
  const 킬로그램 = `${(weight / 1e6).toFixed(2)}kg`;
  const 그램 = `${(weight / 1000).toFixed(2)}g`;
  if (select[2].options[1].selected) rightInput.value = 킬로그램;
  if (select[2].options[2].selected) rightInput.value = 그램;
};
